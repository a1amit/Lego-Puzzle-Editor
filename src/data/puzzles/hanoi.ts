...
export const TOWER_OF_HANOI_PUZZLE: PuzzleDefinition = {
  ...
  validation_rules: [
    ...
    { type: "ROTATION", rule: "NO_ROTATION" },
    {
      ...
      params: {
        label: "Hanoi goal",
        description: "All three disks must end up centered on the rightmost peg.",
        condition: {
          kind: "custom_code",
          code: [
            ...
            "function covers(b, x, y) {",
            "  const s = SIDES[b.id] || 1;",
            "  return b.x <= x && x < b.x + s && b.y <= y && y < b.y + s;",
            "}",
            "const onPegC = board.placedBricks.every(b => covers(b, PEG_C_X, BOARD_CENTER_Y));",
